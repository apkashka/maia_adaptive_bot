from .utils import *
from .main import *
from .opening_book import get_opening_book_or_fallback
...
class TestDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Save original ELO before preprocessing
        original_elo_self = elo_self
        original_elo_oppo = elo_oppo

        board_input, elo_self, elo_oppo, legal_moves = preprocessing(fen, elo_self, elo_oppo, self.elo_dict,
                                                                     self.all_moves_dict)

        return fen, board_input, elo_self, elo_oppo, legal_moves, original_elo_self, original_elo_oppo
...
def get_preds(model, dataloader, all_moves_dict_reversed):
    ...
    with torch.no_grad():
        ...
        for fens, boards, elos_self, elos_oppo, legal_moves, original_elos_self, original_elos_oppo in dataloader:
            ...
            for i in range(len(fens)):
                ...
                move_probs_each, _ = get_opening_book_or_fallback(fen, move_probs_each,
                                                                  elo_self=original_elos_self[i].item(),
                                                                  max_halfmoves=10)
